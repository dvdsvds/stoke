...
- **Ad hoc `stoke run`** — pass an actual file path instead of a target name and stoke runs it on the spot: a C/C++ source file gets built/linked against its target's other non-`main()` sources, and a Go file (e.g. `stoke run cmd/colorpicker-preview/main.go` in a multi-binary `cmd/*/` layout) has its package directory handed straight to `go run` — no target declaration needed for Go, and no scanning for `main()` either way (the compiler catches a missing/duplicate one on its own)
...
- **`stoke self-update [--check] [--yes]`** — update stoke in place, however it was installed. Standalone binary: Linux/macOS swap the install directory atomically (with rollback on failure), Windows launches the installer silently in the background. `pip install -e .` (a git checkout): `git pull` (refuses if the working tree has uncommitted changes). Regular `pip install`: `pip install --upgrade` pointed at the matching GitHub release tag (not published to PyPI)
- **`stoke cache-server --user X --password Y [--port] [--dir] [--host] [--cert] [--key] [--max-upload-mb]`** — a reference server for `STOKE_REMOTE_CACHE_URL`: a small file-backed HTTP key-value store, no database. Secure by default: auth is mandatory (refuses to start without it), binds to `127.0.0.1` unless told otherwise, write-once (rejects overwriting an existing cache key — 409, not silent replacement), capped upload size, and optional built-in TLS via `--cert`/`--key`
