...
# stoke test용: 파일에 main()이 있는지 휴리스틱으로 판단 (주석/문자열까지 파싱하진 않음).
_CPP_MAIN_RE = re.compile(r"\b(?:int|void)\s+main\s*\(")

_DOCTEST_MAIN_CPP = (
    '// Auto-generated by stoke -- provides doctest\'s main() so test files\n'
    '// only need "TEST_CASE(...)" and "#include <doctest.h>".\n'
...
class CBaseAdapter(BaseAdapter):
    ...
    def _has_main(self, path: Path) -> bool:
        try:
    ...
    def _collect_test_files(self) -> list[Path]:
        """test_sources 패턴에서 소스 파일 목록 수집 (collect_source_files와 동일한 규칙)."""
        collected = []
        seen = set()
        for pattern in self.target.test_sources:
            for path in self.project_root.glob(pattern):
                if not path.is_file() or path.suffix != self.source_extension:
                    continue
                try:
                    path.relative_to(self.project_root / ".stoke")
                    continue
    ...
    def test(self, verbose: bool = False) -> int:
        ...
        if self.compiler_kind != "cpp":
            raise RuntimeError(
                ...
                "  Write tests in C++ instead (language = \"cpp\", test_sources + doctest works out of the box),\n"
                "  or set build_system = \"cmake\" or \"meson\" and bring your own C test setup (ctest/meson test)."
            )
        if not self.target.test_sources:
            raise RuntimeError(
                "No 'test_sources' configured for this target in stoke.toml.\n"
                f"  Add e.g. test_sources = [\"tests/*.cpp\"] under [targets.{self.target.name}]"
            )
        ...
        # 애플리케이션 소스 중 main()이 없는 것들(라이브러리 코드)만 테스트 바이너리에 같이 링크.
        # main()이 있는 파일을 같이 넣으면 doctest가 생성하는 main()과 충돌(다중 정의)함.
        lib_sources = [f for f in self.collect_source_files() if not self._has_main(f)]

        profile_name = self.profile.name if self.profile else "default"
        test_dir = self.project_root / ".stoke" / "cpp-tests" / self.target.name / profile_name
        test_dir.mkdir(parents=True, exist_ok=True)

        doctest_include = Path(__file__).resolve().parent / "vendor"
        main_cpp = test_dir / "_stoke_doctest_main.cpp"
        main_cpp.write_text(_DOCTEST_MAIN_CPP, encoding="utf-8")

        exe_name = "test_runner.exe" if platform.system() == "Windows" else "test_runner"
        exe_path = test_dir / exe_name
        all_sources = lib_sources + test_files + [main_cpp]

        print(f"Compiling {len(test_files)} test file(s) with doctest...")
        standard = self._get_standard()
        if is_msvc:
            cmd = [str(compiler.executable), "/nologo", "/utf-8", "/EHsc"]
            if standard:
                cmd.append(f"/std:{standard}")
            cmd.append(f"/I{doctest_include}")
            for inc in self._include_dirs(compiler):
                cmd.append(f"/I{inc}")
            cmd.extend(str(f) for f in all_sources)
            cmd.append(f"/Fe{exe_path}")
            cmd.append("/link")
        else:
            cmd = [str(compiler.executable)]
            if standard:
                cmd.append(f"-std={standard}")
            cmd.extend(["-I", str(doctest_include)])
            for inc in self._include_dirs(compiler):
        ...
        if proc.returncode != 0:
            raise RuntimeError(f"Test build failed:\n{proc.stderr.strip() or proc.stdout.strip()}")

        run_cmd = [str(exe_path)]
        if verbose:
            run_cmd.append("--success")
