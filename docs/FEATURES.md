...
- One target per project — `stoke.toml` supports exactly one `[targets.*]`, so a monorepo of several services (a backend + a worker, say) needs a separate `stoke.toml` per service rather than one shared file.
